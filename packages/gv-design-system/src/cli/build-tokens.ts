...
function varLine(source: Json, name: string, path: string): string {
  const value = getPath(source, path);
  if (value === undefined) throw new Error(`Unresolved token path: ${path}`);
  return `  ${name}: ${String(value)};`;
}

function generateTokensCss(tokens: Json): string {
  const source: Json = { ...tokens, ...STATIC_SCALE };
  ...
  const spacing = tokens.spacing as Json;
 ...
 *   - Canon:    ${String((tokens.meta as Json).canon)}
...
function generateTokensTs(tokens: Json): string {
  const g = (path: string): string => {
    const v = getPath(tokens, path);
    if (v === undefined) throw new Error(`Unresolved token path: ${path}`);
    return String(v);
  };
  const s = (path: string): string => q(g(path));
  const n = (path: string): string => String(getPath(tokens, path));
...
function generateTailwindConfig(tokens: Json): string {
  const colors: Record<string, string> = {};
  for (const [k, v] of Object.entries(flatten(tokens.color as Json))) {
...
module.exports = {
  ...
  theme: {
    extend: {
      ...
      fontSize: ${JSON.stringify((tokens.typography as Json).size, null, 2)},
      borderRadius: ${JSON.stringify(tokens.radius, null, 2)},
      boxShadow: ${JSON.stringify(tokens.shadow, null, 2)},
      transitionTimingFunction: ${JSON.stringify((tokens.motion as Json).easings, null, 2)},
...
function generateFigmaTokens(tokens: Json): string {
  const figma: Json = {
  ...
  figma.surfaceSet = {};
  for (const [k, v] of Object.entries(tokens.surface as Json)) {
    (figma.surfaceSet as Json)[k] = { $type: 'color', $value: v, $description: `surface.${k}` };
  }
  figma.typography = {
...
function generateCssModulesTypes(css: string): string {
...
function validate(tokens: Json): string[] {
  const errors: string[] = [];
  ...
  if (tokens.color) {
    for (const k of ['brand', 'surface', 'text', 'feedback', 'border', 'glow', 'gradient']) {
      if (!(tokens.color as Json)[k]) errors.push(`Missing color.${k}`);
    }
  }

  // Hex sanity: any string that starts with '#' must be a 6-digit hex.
  const hexPattern = /^#[0-9a-f]{6}$/i;
  (function checkHex(obj: Json, path = '') {
    for (const [k, v] of Object.entries(obj)) {
  ...
  const meta = tokens.meta as Json | undefined;
  if (meta) {
